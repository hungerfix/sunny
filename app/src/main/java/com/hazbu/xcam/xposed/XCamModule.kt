...
import com.hazbu.xcam.core.audio.AudioDecoder
...
class XCamModule : XposedModule() {
    ...
    private val audioDecoder = AudioDecoder { printLog(it) }
    ...
    // Audio Delegation
    /**
     * Starts the [AudioDecoder] for the current media path.
     * Called by [AudioHook] after AudioRecord.startRecording() fires.
     *
     * @param path       the virtual media path or URL
     * @param sampleRate the sample rate the app requested from AudioRecord (logged only)
     * @param channels   the channel count the app requested from AudioRecord (logged only)
     */
    fun startAudio(path: String, sampleRate: Int, channels: Int) {
        logHook("[AUDIO] Starting decoder — path=$path SR=$sampleRate CH=$channels")
        audioDecoder.start(path)
    }

    /** Stops the [AudioDecoder]. Called on AudioRecord.stop() / release(). */
    fun stopAudio() = audioDecoder.stop()

    /** Returns true when the [AudioDecoder] has a running decode thread. */
    fun isAudioActive(): Boolean = audioDecoder.isActive

    /**
     * Fills [dest] with virtual PCM from the [AudioDecoder].
     * Returns the number of bytes written (always == [sizeInBytes]).
     */
    fun readAudioData(dest: ByteArray, offsetInBytes: Int, sizeInBytes: Int): Int =
        audioDecoder.read(dest, offsetInBytes, sizeInBytes)
